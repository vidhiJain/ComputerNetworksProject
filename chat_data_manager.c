...
static int callback_socket(void *data, int argc, char **argv, char **azColName){
   int i;
   fprintf(stderr, "%s: ", (const char*)data);
   for(i=0; i<argc; i++){
      printf("%s = %s\n", azColName[i], argv[i] ? argv[i] : "NULL");
   }
   snprintf(socket,sizeof(passw),"%s",argv[i-1]);
   printf("\n");
   return 0;
}
...
int update_socket(char* username,int sock)
{
   ...
   snprintf(sql,sizeof(sql),"%s%d%s%s%s","UPDATE USER_SOCKS set socket = \"",sock,"\"WHERE NAME IS \"",username,"\"");      
   /* Execute SQL statement */
...
int retrieve_socket(char* username)
{
   ...
   snprintf(sql,sizeof(sql),"%s%s%s","SELECT socket from USER_SOCKS WHERE NAME IS\"",username,"\"");
   /* Execute SQL statement */
   rc = sqlite3_exec(db, sql, callback_socket, (void*)data, &zErrMsg);
   if( rc != SQLITE_OK ){
      fprintf(stderr, "SQL error: %s\n", zErrMsg);
      sqlite3_free(zErrMsg);
   }else{
